...
export function PageTitle(props: React.PropsWithChildren) {
  return (
    <h1
      className={
        'font-heading text-foreground text-base leading-none font-bold tracking-tight'
      }
    >
      {props.children}
    </h1>
...
function getSlotsFromPage(props: React.PropsWithChildren) {
  return React.Children.toArray(props.children).reduce<{
    ...
    MobileNavigation: React.ReactNode | null;
  }>(
    (acc, child) => {
      ...
      if (
        child.type === PageNavigation ||
        (typeof child.type === 'function' &&
          child.type.name === 'PageNavigation')
      ) {
        return {
          ...acc,
          Navigation: child,
      ...
      if (
        child.type === PageMobileNavigation ||
        (typeof child.type === 'function' &&
          child.type.name === 'PageMobileNavigation')
      ) {
        return {
          ...acc,
    ...
    },
    {
      ...
      Navigation: null as React.ReactNode,
      MobileNavigation: null as React.ReactNode,
    },
  );
}
